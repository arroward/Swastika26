...
export default function WhoShouldAttend() {
    return (
        <section className="py-24 px-6 md:px-12 bg-white/5 mx-6 md:mx-12 rounded-[3rem] relative overflow-hidden">
            ...
            <div className="absolute inset-0 bg-gradient-to-b from-[var(--accent-blue)]/5 to-transparent pointer-events-none" />
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 max-w-[1600px] mx-auto relative z-10">
                {targets.map((t, i) => (
                    <div key={i} className="group h-[300px] md:h-[400px] bg-black border border-white/10 rounded-2xl flex flex-col items-center justify-center gap-6 p-6 hover:border-[var(--accent-blue)] transition-colors duration-500 relative overflow-hidden cursor-default">

                        {/* Hover Glow */}
                        <div className="absolute inset-0 bg-[var(--accent-blue)] opacity-0 group-hover:opacity-10 transition-opacity duration-500" />

                        <div className="bg-white/5 p-6 rounded-full text-white group-hover:scale-110 group-hover:bg-[var(--accent-blue)] group-hover:text-white transition-all duration-500">
                            {t.icon}
                        </div>
