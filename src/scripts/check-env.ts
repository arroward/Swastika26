        ...
        try {
            ...
            if (process.env.R2_BUCKET_NAME) {
                try {
                    ...
                    console.log(`  ✅ R2 Connection successful & Bucket '${process.env.R2_BUCKET_NAME}' found.`);
                } catch (bucketErr: any) {
                    // If HeadBucket fails, it might be 403 or 404
                    console.log(`  ❌ R2 Connected but Bucket check failed: ${bucketErr.message}`);
                    allPassed = false;
                }
            } else {
    ...
    if (process.env.FIREBASE_PROJECT_ID && process.env.FIREBASE_CLIENT_EMAIL && process.env.FIREBASE_PRIVATE_KEY) {
        ...
        try {
            ...
            try {
                ...
                console.log('  ✅ Firebase Admin initialized & Auth service reachable.');
            } catch (innerErr: any) {
                if (innerErr.code === 'auth/insufficient-permission') {
                    console.log('  ⚠️  Firebase Admin initialized but has limited permissions (Auth list failed). Credentials likely valid.');
                } else if (innerErr.message.includes('credential')) {
                    console.log(`  ❌ Firebase Admin credentials invalid: ${innerErr.message}`);
                    allPassed = false;
                } else {
                    // Other errors might be network or project config. Warning is safer than fail if we aren't sure.
                    console.log(`  ⚠️  Firebase Admin initialized but encountered error: ${innerErr.message}`);
                }
            }
