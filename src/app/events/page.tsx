...
export default function EventsPage() {
    return (
        <main className="bg-[var(--bg-main)] min-h-screen text-white pt-24 px-6 md:px-24 pb-20 overflow-x-hidden selection:bg-[var(--accent-blue)] selection:text-white">
                ...
                <div className="grid grid-cols-1 gap-8">
                    {events.map((event, i) => (
                        <RevealOnScroll key={event.slug} delay={i * 0.1}>
                            <div className="group relative rounded-3xl overflow-hidden border border-white/10 bg-[#0a0a0a] hover:border-[var(--accent-blue)]/50 transition-all hover:bg-white/5">
                                <div className="flex flex-col md:flex-row h-full">
                                    ...
                                    <div className="md:w-1/3 aspect-video md:aspect-auto overflow-hidden relative">
                                        ...
                                        <div className="absolute top-4 left-4 z-20 bg-black/60 backdrop-blur-md px-3 py-1 rounded-full text-xs font-bold border border-white/10 text-[var(--accent-blue)]">
                                            {event.category}
                                        </div>
                                    </div>
                                    ...
                                    <div className="md:w-2/3 p-8 flex flex-col justify-center">
                                        <div className="flex flex-wrap gap-6 mb-4 text-sm text-[var(--text-secondary)] font-medium">
                                            <div className="flex items-center gap-2"><Calendar size={14} className="text-[var(--accent-blue)]" /> {event.date}</div>
                                            <div className="flex items-center gap-2"><Clock size={14} className="text-[var(--accent-blue)]" /> {event.time}</div>
                                            <div className="flex items-center gap-2"><MapPin size={14} className="text-[var(--accent-blue)]" /> {event.venue}</div>
                                        </div>

                                        <h2 className="text-3xl md:text-4xl font-display font-bold mb-3 group-hover:text-[var(--accent-blue)] transition-colors">{event.title}</h2>
