...
export default function EventDetailPage() {
        ...
        return (
            <div className="min-h-screen bg-[var(--bg-main)] text-white flex items-center justify-center">
                <div className="text-center">
                    ...
                    <Link href="/events" className="text-[var(--accent-blue)] hover:underline">Back to Events</Link>
                </div>
            </div>
        );
    }
    ...
    return (
        <main className="bg-[var(--bg-main)] min-h-screen text-white overflow-x-hidden selection:bg-[var(--accent-blue)] selection:text-white pb-20">

            {/* Navbar */}
            <nav className="fixed top-0 left-0 w-full z-50 px-6 py-6 flex items-center justify-between pointer-events-none">
                <Link href="/" className="pointer-events-auto font-display font-bold text-2xl tracking-tighter mix-blend-difference">Swastika<span className="text-[var(--accent-blue)]">.</span></Link>
            ...
            <section className="relative h-[80vh] flex items-end pb-24 px-6 md:px-24">
                ...
                <div className="relative z-10 w-full max-w-6xl">
                    <RevealOnScroll>
                        <div className="inline-flex items-center gap-2 border border-white/10 bg-white/5 backdrop-blur-sm px-4 py-1.5 rounded-full text-xs uppercase tracking-widest text-[var(--accent-blue)] font-bold mb-6">
                        ...
                        <div className="flex flex-wrap gap-8 text-sm font-bold tracking-widest uppercase">
                            <div className="flex items-center gap-2"><Calendar className="text-[var(--accent-blue)]" size={18} /> {event.date}</div>
                            <div className="flex items-center gap-2"><Clock className="text-[var(--accent-blue)]" size={18} /> {event.time}</div>
                            <div className="flex items-center gap-2"><MapPin className="text-[var(--accent-blue)]" size={18} /> {event.venue}</div>
                        </div>
                    </RevealOnScroll>
                </div>
            </section>
            ...
            <div className="px-6 md:px-24 max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-16">
                ...
                <div className="md:col-span-2 space-y-16">
                    ...
                    <RevealOnScroll>
                        <section>
                            <h3 className="text-2xl font-display font-bold mb-6 flex items-center gap-3"><FileText className="text-[var(--accent-blue)]" /> Description</h3>
                    ...
                    <RevealOnScroll>
                        <section>
                            <h3 className="text-2xl font-display font-bold mb-6 flex items-center gap-3"><AlertCircle className="text-[var(--accent-blue)]" /> Rules & Guidelines</h3>
                            <ul className="grid gap-4">
                                {event.rules.map((rule, i) => (
                                    <li key={i} className="flex items-start gap-4 p-4 rounded-xl bg-white/5 border border-white/10 hover:border-[var(--accent-blue)]/30 transition-colors">
                                        <span className="w-6 h-6 rounded-full bg-[var(--accent-blue)]/20 text-[var(--accent-blue)] flex items-center justify-center text-xs font-bold mt-0.5">{i + 1}</span>
                    ...
                    {event.timeline && (
                        <RevealOnScroll>
                            <section>
                                <h3 className="text-2xl font-display font-bold mb-6 flex items-center gap-3"><Clock className="text-[var(--accent-blue)]" /> Event Flow</h3>
                                <div className="border-l-2 border-white/10 ml-3 pl-8 space-y-8">
                                    {event.timeline.map((item: any, i: number) => (
                                        <div key={i} className="relative">
                                            <div className="absolute -left-[39px] w-5 h-5 rounded-full bg-[var(--bg-main)] border-2 border-[var(--accent-blue)] z-10 box-content"></div>
                    ...
                    {event.prizes && (
                        <RevealOnScroll>
                            <section>
                                <h3 className="text-2xl font-display font-bold mb-6 flex items-center gap-3"><Trophy className="text-[var(--accent-blue)]" /> Rewards</h3>
                ...
                <div className="space-y-8">
                    ...
                    <RevealOnScroll delay={0.2}>
                        <div className="p-8 rounded-3xl bg-white/5 border border-white/10 space-y-6">
                            ...
                            <button className="w-full bg-[var(--accent-blue)] hover:bg-blue-600 text-white font-bold py-4 rounded-xl transition-all hover:scale-105 shadow-[0_0_20px_-5px_var(--accent-blue)] flex items-center justify-center gap-2">
                            ...
                            {event.coordinator && (
                                <div className="flex items-start gap-4">
                                    ...
                                    <div>
                                        ...
                                        <a href={`mailto:${event.coordinator.contact}`} className="text-sm flex items-center gap-2 hover:text-white text-[var(--accent-blue)] transition-colors">
